...
contract Dapp is MyToken {
    ...
    event Deposited(address indexed user, uint256 amount, uint8 tier, uint256 reward, uint256 unlockTime);
    ...
    modifier hasDeposit(uint8 _tier) {
        if (deposits[msg.sender][_tier].amount == 0) revert NoActiveDeposit(_tier);
        _;
    }

    // Modifier to ensure the deposit cooldown period has passed before allowing another deposit
    modifier canDeposit() {
        if (block.timestamp < lastDepositTime[msg.sender] + depositCooldown) revert DepositCooldownNotReached();
        _;
    }
    ...
    function updateTier(uint8 _tier, uint256 _rewardRate, uint256 _lockTime) external onlyOwner {
    ...
    function deposit(uint8 _tier, uint256 _amount) external payable onlyWhitelisted canDeposit {
        ...
        deposits[msg.sender][_tier] = Deposit({
            ...
            unlockTime: userDeposit.unlockTime + block.timestamp + tiers[_tier].lockTime,
            claimed: false
        });

        lastDepositTime[msg.sender] = block.timestamp; // Update the last deposit time

        emit Deposited(msg.sender, _amount, _tier, reward, block.timestamp + tiers[_tier].lockTime); // Emit a deposit event
    }

    // Function to claim the deposited amount and its reward after the lock period
    function claim(uint8 _tier) external onlyWhitelisted hasDeposit(_tier) canClaim(_tier) {
        Deposit storage userDeposit = deposits[msg.sender][_tier]; // Get the user's deposit for the tier
        if (block.timestamp <= userDeposit.unlockTime) revert DepositLocked(userDeposit.unlockTime); // Reject if the deposit is still locked
    ...
    function getDepositDetails(address _user, uint8 _tier) external view returns (uint256, uint256, uint256, bool) {
        Deposit memory userDeposit = deposits[_user][_tier]; // Get the deposit details
        return (userDeposit.amount, userDeposit.reward, userDeposit.unlockTime, userDeposit.claimed); // Return the details
    }
